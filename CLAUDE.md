...
- `src/container-runner.ts` — Spawns `docker run -i --rm` with living file + IPC + skills mounts (`--mount type=bind`, so the daemon rejects a missing source atomically; ro/rw split enforced per file), hardening flags (`--init --cap-drop=ALL --security-opt no-new-privileges --pids-limit`, opt-in `--memory`/`--cpus`), collision-proof names (`kuchiclaw-<group>-<ts>-<hex>`), and a host-set `kuchiclaw.owner` label (orchestrator|cli). Passes ContainerInput via stdin, parses sentinel markers from stdout. Timeout runs a `running → terminating → settled` state machine: kill-by-name via the daemon, a stdout-drain latch so a late flush isn't truncated, then adjudication — parseable output resolves success (idle cleanup, no retry), confirmed death rejects retryable, unconfirmed death raises a containment failure (typed non-retryable error + orchestrator restart). Finalizer never throws; `newTokens` persist on every settle path
...
- `src/config.ts` — Constants: image name, sentinel markers, timeout, paths, queue config, IPC config, skills/MCP paths, scheduler poll interval, MAIN_CHAT_ID, ALLOWED_SENDER_IDS, and model selection (`AGENT_MODEL`=opus / `AGENT_FALLBACK_MODEL`=sonnet aliases, env-overridable; `API_KEY_MODEL` for the pay-per-token path). The fallback is load-bearing: the Max Opus weekly cap is shared with the operator's coding sessions
...
- `container/entrypoint.ts` — Runs inside Docker: exports `runEntrypoint(raw, deps)` (the full flow — parse → secrets → refresh+scrub → SDK `query()` loop → emit between markers) with an injected env/observe seam so host vitest tests the real path with a fake `query`; the production shell dynamically imports the SDK. The OAuth **refresh token is never agent-visible** — refreshed (with response validation) and scrubbed from the input before the SDK starts; only the short-lived access token enters env
