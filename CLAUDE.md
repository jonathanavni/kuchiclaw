...
M0 (scaffolding), M1 (basic agent loop), M2 (persistent context + web tools), M3 (SQLite + message history), M4 (Telegram integration), M5 (orchestrator + queue), M6 (IPC + skills system), M7 (scheduled tasks + heartbeat), M8 (multi-group isolation), M9 (deploy to Hetzner), M10 (crash recovery), and M11 (living file backup via git) are complete. **M9 + M10 + M11 = MVP.** M12 (hardening) phases 1–6 are shipped and deployed (only P7 test/doc debt remains): IPC boundary, container lifecycle, resilience, auth lineage/secret scoping, deps/model/deploy (Node 24, better-sqlite3 13, SDK 0.3.x, explicit opus+sonnet-fallback model config, fail-closed sender allowlist), and result transport & context hygiene (HMAC-signed result file replacing stdout sentinels, TZ-aware time/cron injection, injected history/memory budgets + structure defense).
